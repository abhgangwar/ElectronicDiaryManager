...
public:
  ...
  {
    int r;
    if (supp.dt[0] == str.dt[0] && supp.dt[1] == str.dt[1]
	&& supp.dt[2] == str.dt[2])
      r = 1;
    else
      r = 0;
  ...
  int wentry (char usr1[], unsigned long &m)
  {
    ...
    puts
      ("                                                     Write An Entry\n");
    puts
      ("                                    Please press ctr+F and Enter to save your entry when finished\n");
    i = 0;
    ofstream ftemp;
...
public:
  ...
  void umenu ()
  {
    ...
    char c;
    cout <<
      "\nPlease pick up a right choice[1,2,3,4]:\n 1.Write a new entry\n 2.View last entry\n 3.View a perticular entry\n 4.Exit\n";
    c = getch ();
    switch (c)
      {
      case '1':
  ...
  if (!ureg (usr))
      ...
      if (!u.checkhash (p))
	{
	  ...
	  if (n <= 3)
	    ...
	    {
	      cout <<
		"Sorry you have attempted maximum times.......press any key to exit\n";
	      getch ();
	      exit (1);
	    }
	}
